...
function setFX(esMovil){
    ...
    if (esMovil){
        ...
        checkActualizar = function(){
          if (Boolean(window.AppUpdate)){
            return;
          }
          const updateUrl = VARS.SERVER_UPDATE_URL;
...
function _fnSepararArregloSegunCriterio(arreglo, fnFiltro){
    //Esta funcion permite filtrar un arreglo en base a uan funcion filtro y luego separarla en dos partes,
    //una que contiene items que cumplen el criterio y otra que no.
    let arregloCumplen = [], arregloNoCumplen = [];
    for (var i = 0; i < arreglo.length; i++) {
        const item = arreglo[i];
        if (fnFiltro(item)){
            arregloCumplen.push(item);
        } else {
            arregloNoCumplen.push(item);
        }
    };

    return {cumplen: arregloCumplen, nocumplen: arregloNoCumplen};
};
